...
from getl.common.utils import fetch_filepaths_from_prefix, extract_bucket_and_prefix
...
class FolderBased(FileRegistry):
    ...
    def _get_files_to_lift(dataframe: DataFrame) -> List[str]:
        ...
        return [
            FileRegistryRow(row.file_path, row.date_lifted)
            for row in data
        ]
    ...
    def _update_file_registry(self, list_of_rows: List[FileRegistryRow]) -> DataFrame:
        ...
        delta_table = DeltaTable(self.file_registry_path, self.spark)
        return delta_table.insert_all(updates_df, 'source.file_path = updates.file_path')
    ...
    def _get_new_s3_files(s3_path: str, suffix: str) -> List[FileRegistryRow]:
        ...
        keys = list(fetch_filepaths_from_prefix(s3_path + '/', suffix, prepend_bucket=True))
    ...
    def _create_file_registry(self,
                              ...
                              rows_of_paths: List[str]) -> DataFrame:
    ...
    def _create_hive_table(self, file_registry_path: str):
        ...
        hive.create(file_registry_path, db_schema="""
            file_path STRING NOT NULL,
            date_lifted TIMESTAMP
        """)

    def _create_file_registry_path(self, s3_path: str) -> str:
        LOGGER.info('Combining base prefix: %s with read path: %s',
        ...
        return 's3://{}'.format(Path(file_registry_bucket) / file_registry_prefix / prefix)
